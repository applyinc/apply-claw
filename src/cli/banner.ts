...
// ---------------------------------------------------------------------------
// APPLYCLAW ASCII art (figlet "ANSI Shadow" font, baked at build time)
// ---------------------------------------------------------------------------
const APPLYCLAW_ASCII = [
  " █████╗ ██████╗ ██████╗ ██╗  ██╗   ██╗ ██████╗██╗      █████╗ ██╗    ██╗",
  "██╔══██╗██╔══██╗██╔══██╗██║  ╚██╗ ██╔╝██╔════╝██║     ██╔══██╗██║    ██║",
  "███████║██████╔╝██████╔╝██║   ╚████╔╝ ██║     ██║     ███████║██║ █╗ ██║",
  "██╔══██║██╔═══╝ ██╔═══╝ ██║    ╚██╔╝  ██║     ██║     ██╔══██║██║███╗██║",
...
export function formatCliBannerArt(options: BannerOptions = {}): string {
  ...
  if (!rich) {
    return APPLYCLAW_ASCII.join("\n");
  }
  return renderGradientFrame(APPLYCLAW_ASCII, 0);
}
...
export async function emitCliBanner(version: string, options: BannerOptions = {}) {
  ...
  if (rich) {
    ...
    process.stdout.write(APPLYCLAW_ASCII.join("\n") + "\n");
  }
