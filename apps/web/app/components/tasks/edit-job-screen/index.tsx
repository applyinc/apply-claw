...
import { useEditJobScreen } from "./hooks";

type Props = {
  jobId: string;
};

export function EditJobScreen(props: Props) {
  const { jobId } = props;
  const { job, loading, saving, initialForm, onSave, onCancel } = useEditJobScreen(props);
      ...
      <div className="mx-auto max-w-3xl px-5 py-7 md:px-7">
        ...
        <Card className="rounded-[24px] border-neutral-200 bg-white shadow-none">
          <CardHeader className="pb-3">
            <CardTitle className="text-[1.65rem] tracking-tight text-neutral-900">
              {job ? job.name : "Job Setup"}
            </CardTitle>
          ...
          <CardContent>
            {loading ? (
              <div className="py-10 text-sm text-neutral-500">Loading job...</div>
            ) : initialForm ? (
              <TaskForm initial={initialForm} onSave={onSave} onCancel={onCancel} saving={saving} />
            ) : (
              <div className="space-y-4 py-4">
                <p className="text-sm text-neutral-500">The requested job could not be found.</p>
                <Button asChild variant="outline" className="rounded-[14px] border-neutral-200 bg-white text-neutral-700 shadow-none hover:bg-neutral-100">
                  <Link href="/tasks">Back to Jobs</Link>
                </Button>
              </div>
            )}
          </CardContent>
