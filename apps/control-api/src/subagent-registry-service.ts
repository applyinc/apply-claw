/**
 * Subagent registry — reads the shared subagent runs.json file.
 * Ported from apps/web/lib/subagent-registry.ts
 */
...
export function resolveSubagentStatus(
  ...
  if (entry.outcome?.status === "error") return "error";
  return "completed";
}
...
export function listSubagentsForRequesterSession(
  ...
  return readSubagentRegistry()
    ...
    .map((entry) => ({ ...entry, status: resolveSubagentStatus(entry) }))
