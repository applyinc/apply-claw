/**
 * Feedback service — captures un-redacted $ai_trace events for PostHog
 * when a user submits thumbs up/down feedback on a chat message.
 */
import { readFileSync, existsSync } from "node:fs";
import { join } from "node:path";

import { resolveWebChatDir } from "./workspace-service.js";
import { trackServer } from "./telemetry-service.js";
 ...
 * Reads the session JSONL and emits a PostHog $ai_trace event.
 */
export function submitFeedback(params: {
  ...
  try {
    const { sessionId, messageId, distinctId } = params;
